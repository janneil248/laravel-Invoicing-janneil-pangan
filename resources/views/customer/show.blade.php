                ...
                <table class="table mb-0">
                    ...
                    @foreach ($customers as $customer)
                        <tr>
                            ...
                            <form action="/users/delete/{{$customer->customer_id}}" method="post">
                                @csrf
                                @method('DELETE')
                                <td><a href="/users/edit/{{$customer->customer_id}}" type="button" class="btn btn-info btn-icon-split btn-sm">
                        ...
                        <tr>
                            <td>{{$customers->customer_username}}</td>
                            <td>{{$customers->customer_firstname.' '.$customers->customer_lastname}}</td>
                            <td>{{$customers->customer_address}}</td>
                            <form action="/customers/delete/{{$customers->customer_id}}" method="post">
                                @csrf
                                @method('DELETE')
                                <td><a href="/customers/edit/{{$customers->customer_id}}" type="button" class="btn btn-info btn-icon-split btn-sm">
                                        <span class="text">Update</span>
                                    </a>
                                    &emsp;
                                    <button type="submit" class="btn btn-danger btn-icon-split btn-sm">
